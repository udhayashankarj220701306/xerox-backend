...
export const getProfile = async (req, res) => {
  try {
    ...
    const xeroxProfile = await Xerox.findOne({ xeroxId }).populate("xeroxId");
    if (!xeroxProfile) {
...
export const updateProfile = async (req, res) => {
  try {
    ...
    const xeroxProfile = await Xerox.findByIdAndUpdate(xeroxId, updates, {
      new: true,
      runValidators: true, // Ensures validation runs (critical for 'required')
